...
public class DatabaseXmlContext {
	private static final Logger logger = LoggerFactory.getLogger(DatabaseXmlContext.class);
	...
	public static void start() throws IOException, PerformanceException {
	    ...
	    PetJdbcDao jdao=new PetJdbcDao();
	    jdao.fetchAll();
      ...
      PetDao dao = (PetDao) ctx.getBean("dayFourDao");
      dao.fetchAll();
...
//      dao.exceptionCreation();
       ...
       */
      PetHibernateDao hdao = (PetHibernateDao) ctx.getBean("hibernateDao");
      Pet p = new Pet();
      p.setOwner("Ashok");
